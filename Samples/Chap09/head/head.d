    ...
    switch (message)
    {
        ...
        case WM_COMMAND:
            ...
            if (LOWORD(wParam) == ID_LIST && HIWORD(wParam) == LBN_DBLCLK)
            {
                ...
                {
                    bValidFile = false;
                    SetCurrentDirectory(std.path.join(getcwd, r"..\").toUTF16z);
                ...
                else if (filename[0] == '[' && filename[$-1] == ']')
                {
                    ...
                    {
                        // dir
                        SetCurrentDirectory(std.path.join(getcwd, filename[1 .. $-1]).toUTF16z);
                        SetWindowText(hwndText, getcwd.toUTF16z);
                        SendMessage(hwndList, LB_RESETCONTENT, 0, 0);
                        SendMessage(hwndList, LB_DIR, DIRATTR, cast(LPARAM)"*.*".toUTF16z);
                    }
                ...
                {
                    bValidFile = true;
                    szFile = std.path.join(getcwd, filename).dup;
